...
const logger = (0, logger_1.setupLogger)('БазаДанных');
...
const db = new sqlite3_1.default.Database(dbPath, (err) => {
    if (err) {
        ...
        db.run('PRAGMA journal_mode = WAL;', (err) => {
            if (err)
                logger.error('Ошибка при установке PRAGMA journal_mode = WAL!', err);
        });
        db.run('PRAGMA synchronous = NORMAL;', (err) => {
            if (err)
                logger.error('Ошибка при установке PRAGMA synchronous = NORMAL!', err);
        });
        db.run('PRAGMA foreign_keys = ON;', (err) => {
            if (err)
                logger.error('Ошибка при установке PRAGMA foreign_keys = ON (ограничения внешних ключей)!', err);
        });
        ...
        status TEXT DEFAULT 'active'
            ...
            db.all(`PRAGMA table_info(lots)`, (err, rows) => {
                if (err)
                    return logger.error('Ошибка при проверке схемы таблицы lots (PRAGMA table_info)!', err);
                const hasColumn = rows.some(r => r.name === 'tv_model_id');
                if (!hasColumn) {
                    db.run(`ALTER TABLE lots ADD COLUMN tv_model_id INTEGER`, (err) => {
                        if (err)
                            logger.error('Ошибка при миграции схемы: не удалось добавить tv_model_id в lots!', err);
                        else
                            logger.info('Успешно выполнена миграция схемы: добавлена колонка tv_model_id в таблицу lots.');
                    });
                }
            });
            ...
            FOREIGN KEY(user_id) REFERENCES users(id)
                    ...
                    db.all(`PRAGMA table_info(iqc_panels_logs)`, (err, rows) => {
                        if (err || !rows)
                            return;
            ...
            FOREIGN KEY(user_id) REFERENCES users(id)
                    ...
                    db.all(`PRAGMA table_info(oqa_tv_logs)`, (err, rows) => {
                        if (err || !rows)
                            return;
                        const hasDefectType = rows.some(r => r.name === 'defect_type');
                        if (hasDefectType) {
            ...
            FOREIGN KEY(user_id) REFERENCES users(id)
                ...
                db.all(`PRAGMA table_info(${mod}_logs)`, (err, rows) => {
                    if (err)
                        return;
        ...
        closed_at TEXT
            ...
            db.all(`PRAGMA table_info(daily_kpi_facts)`, (err, rows) => {
                if (err || !rows)
                    return;
                ...
                if (!hasClosedAt) {
                    db.run(`ALTER TABLE daily_kpi_facts ADD COLUMN closed_at TEXT`, (err) => {
                        if (err)
                            console.error('[Migration] Failed to add closed_at column to daily_kpi_facts:', err.message);
                        else
                            console.log('[Migration] Added closed_at column to daily_kpi_facts');
                    });
                }
            });
            // Audit logs disabled
