...
const logger = (0, logger_1.setupLogger)('Бэкенд');
...
const storage = multer_1.default.diskStorage({
    ...
    filename: (req, file, cb) => {
        ...
        cb(null, (req.params.module || 'file') + '-' + uniqueSuffix + path_1.default.extname(file.originalname));
    }
});
...
const getSetting = (key) => {
    return new Promise((resolve) => {
        db_1.default.get('SELECT value FROM global_settings WHERE key = ?', [key], (err, row) => {
...
const authenticateToken = (req, res, next) => {
    ...
    if (apiKey) {
        db_1.default.get(`SELECT value FROM global_settings WHERE key = 'api_key'`, (err, row) => {
            if (!err && row && row.value && row.value === apiKey) {
                req.user = { id: 0, username: 'API_SYSTEM', role: 'Admin' };
                return next();
            }
            if (isPublicPath)
                return next();
...
const checkModulePermission = (req, res, next) => {
    ...
    if (user.role === 'Admin')
        return next();
    db_1.default.get('SELECT permissions FROM users WHERE id = ?', [user.id], (err, row) => {
        if (err || !row)
...
app.post('/api/auth/login', (req, res) => {
    ...
    db_1.default.get('SELECT * FROM users WHERE username = ?', [username], (err, user) => __awaiter(void 0, void 0, void 0, function* () {
        if (err)
            return res.status(500).json({ error: 'Database error' });
        if (!user)
...
app.post('/api/auth/verify', authenticateToken, (req, res) => {
    ...
    db_1.default.get('SELECT password FROM users WHERE id = ?', [userId], (err, user) => __awaiter(void 0, void 0, void 0, function* () {
        if (err || !user)
            return res.status(500).json({ error: 'Database error' });
...
app.get('/api/users', authenticateToken, (req, res) => {
    db_1.default.all('SELECT id, username, role, permissions FROM users', [], (err, rows) => {
        if (err)
            return res.status(500).json({ error: err.message });
        const parsedRows = rows.map(r => {
            try {
                return Object.assign(Object.assign({}, r), { permissions: JSON.parse(r.permissions) || [] });
            }
            catch (e) {
                return Object.assign(Object.assign({}, r), { permissions: [] });
            }
        });
        res.json(parsedRows);
    });
});
app.post('/api/users', authenticateToken, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { username, password, role, permissions } = req.body;
    const hashedPassword = yield bcryptjs_1.default.hash(password, 10);
    const permsStr = JSON.stringify(permissions || []);
    db_1.default.run('INSERT INTO users (username, password, role, permissions) VALUES (?, ?, ?, ?)', [username, hashedPassword, role, permsStr], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
        res.json({ id: this.lastID, username, role, permissions: permissions || [] });
    });
}));
app.put('/api/users/:id', authenticateToken, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { username, password, role } = req.body;
    try {
        if (password) {
            const hashedPassword = yield bcryptjs_1.default.hash(password, 10);
            db_1.default.run('UPDATE users SET username = ?, password = ?, role = ? WHERE id = ?', [username, hashedPassword, role, req.params.id], function (err) {
                if (err)
                    return res.status(500).json({ error: err.message });
                res.json({ success: true });
            });
            ...
            db_1.default.run('UPDATE users SET username = ?, role = ? WHERE id = ?', [username, role, req.params.id], function (err) {
                if (err)
                    return res.status(500).json({ error: err.message });
                res.json({ success: true });
            });
...
app.put('/api/users/:id/permissions', authenticateToken, (req, res) => {
    ...
    const permsStr = JSON.stringify(permissions || []);
    db_1.default.run('UPDATE users SET permissions = ? WHERE id = ?', [permsStr, req.params.id], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
        res.json({ success: true });
    });
...
app.delete('/api/users/:id', authenticateToken, (req, res) => {
    ...
    db_1.default.serialize(() => {
        modules.forEach(mod => {
            db_1.default.run(`UPDATE ${mod}_logs SET user_id = NULL WHERE user_id = ?`, [userId]);
        });
        db_1.default.run('DELETE FROM users WHERE id = ?', [userId], function (err) {
            if (err)
                return res.status(500).json({ error: err.message });
            res.json({ success: true });
        });
...
app.get('/api/lots', authenticateToken, (req, res) => {
    ...
    if (userRole === 'Admin' || userId === 0) {
        db_1.default.all('SELECT * FROM lots ORDER BY id DESC', [], (err, rows) => {
            if (err)
                return res.status(500).json({ error: err.message });
            res.json(rows);
        });
        ...
        db_1.default.get('SELECT permissions FROM users WHERE id = ?', [userId], (err, row) => {
            let permissions = [];
            ...
            db_1.default.all(query, [], (err2, rows) => {
                if (err2)
...
app.post('/api/lots', authenticateToken, (req, res) => {
    ...
    db_1.default.run('INSERT INTO lots (name, tv_model_id) VALUES (?, ?)', [name, tv_model_id], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
        logAudit(req.user.id, 'CREATE_LOT', { name, tv_model_id });
...
app.put('/api/lots/:id', authenticateToken, (req, res) => {
    ...
    db_1.default.run('UPDATE lots SET name = ?, tv_model_id = ?, status = ? WHERE id = ?', [name, tv_model_id, status || 'active', req.params.id], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
...
app.delete('/api/lots/:id', authenticateToken, (req, res) => {
    ...
    db_1.default.serialize(() => {
        modules.forEach(mod => {
            db_1.default.run(`DELETE FROM ${mod}_logs WHERE lot_id = ?`, [lotId]);
        });
        db_1.default.run('DELETE FROM lots WHERE id = ?', [lotId], function (err) {
            if (err)
                return res.status(500).json({ error: err.message });
...
app.put('/api/logs/:module/:id', authenticateToken, checkModulePermission, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const module = req.params.module;
    ...
    try {
        ...
        const oldRecord = yield new Promise((resolve) => {
            db_1.default.get(`SELECT * FROM ${module}_logs WHERE id = ?`, [id], (err, row) => resolve(row));
        });
        const result = yield new Promise((resolve, reject) => {
            db_1.default.run(`UPDATE ${module}_logs SET data = ?, status = ? WHERE id = ?`, [JSON.stringify(parsedData), status, id], function (err) {
                if (err)
                    reject(err);
                else
                    resolve(this);
...
app.delete('/api/logs/:module/:id', authenticateToken, checkModulePermission, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const module = req.params.module;
    const id = req.params.id;
    const user_id = req.user.id;
    if (!VALID_MODULES.includes(module))
    ...
    try {
        ...
        yield new Promise((resolve, reject) => {
            db_1.default.run(`DELETE FROM ${module}_logs WHERE id = ?`, [id], (err) => {
                if (err)
                    reject(err);
                else
                    resolve();
...
app.get('/api/logs/oqa_labels/last-success', authenticateToken, (req, res) => {
    db_1.default.get("SELECT timestamp FROM oqa_labels_logs WHERE status = 'OK' ORDER BY id DESC LIMIT 1", [], (err, row) => {
        if (err)
            return res.status(500).json({ error: err.message });
...
app.get('/api/logs/:module', authenticateToken, checkModulePermission, (req, res) => {
  ...
  `;
    ...
    db_1.default.all(query, params, (err, rows) => {
        if (err)
            return res.status(500).json({ error: err.message });
        const parsed = rows.map(r => (Object.assign(Object.assign({}, r), { data: safeJsonParse(r.data) })));
        res.json(parsed);
    });
});
app.post('/api/logs/:module', authenticateToken, checkModulePermission, (req, res) => {
    ...
    db_1.default.run(`INSERT INTO ${module}_logs (lot_id, user_id, date, data, status, timestamp) VALUES (?, ?, ?, ?, ?, ?)`, [lot_id, user_id, date, dataStr, status, ts], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
...
app.get('/api/metrics', authenticateToken, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (date && typeof date === 'string') {
        ...
        if (parts.length === 3)
            ruDate = `${parts[2]}.${parts[1]}.${parts[0]}`;
    }
    try {
      ...
      `;
        ...
        const results = yield Promise.all(queries.map(q => new Promise((resolve, reject) => {
            db_1.default.get(q.query, q.params, (err, row) => {
                if (err)
                    reject(err);
                else
                    resolve(row);
...
app.get('/api/suppliers', authenticateToken, (req, res) => {
    db_1.default.all('SELECT * FROM suppliers', [], (err, rows) => {
        if (err)
            return res.status(500).json({ error: err.message });
        res.json(rows);
    });
...
app.post('/api/suppliers', authenticateToken, (req, res) => {
    const { name } = req.body;
    db_1.default.run('INSERT INTO suppliers (name) VALUES (?)', [name], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
...
app.put('/api/suppliers/:id', authenticateToken, (req, res) => {
    ...
    db_1.default.run('UPDATE suppliers SET name = ?, is_active = ? WHERE id = ?', [name, is_active === undefined ? 1 : is_active, req.params.id], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
        res.json({ success: true });
    });
...
app.delete('/api/suppliers/:id', authenticateToken, (req, res) => {
    ...
    db_1.default.serialize(() => {
        db_1.default.run('DELETE FROM articles WHERE supplier_id = ?', [supplierId]);
        db_1.default.run('DELETE FROM suppliers WHERE id = ?', [supplierId], function (err) {
            if (err)
                return res.status(500).json({ error: err.message });
            res.json({ success: true });
        });
...
app.post('/api/articles', authenticateToken, (req, res) => {
    ...
    db_1.default.run('INSERT INTO articles (supplier_id, name, category, specs, drawing_url) VALUES (?, ?, ?, ?, ?)', [supplier_id, name, category || 'General', specs || '', drawing_url || ''], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
...
app.post('/api/articles/bulk', authenticateToken, (req, res) => {
    ...
    db_1.default.serialize(() => {
        const stmt = db_1.default.prepare('INSERT INTO articles (supplier_id, name, category) VALUES (?, ?, ?)');
        articles.forEach((art) => stmt.run(supplier_id, art.name, art.category || 'General'));
        stmt.finalize();
        res.json({ success: true });
    });
});
app.put('/api/articles/:id', authenticateToken, (req, res) => {
    ...
    db_1.default.run(`UPDATE articles SET name = ?, category = ?, specs = ?, drawing_url = ?, is_active = ? WHERE id = ?`, [name, category, specs, drawing_url, is_active === undefined ? 1 : is_active, req.params.id], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
        res.json({ success: true });
    });
...
app.delete('/api/articles/:id', authenticateToken, (req, res) => {
    db_1.default.run('DELETE FROM articles WHERE id = ?', [req.params.id], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
        res.json({ success: true });
    });
...
app.post('/api/components-master', authenticateToken, (req, res) => {
    ...
    db_1.default.run('INSERT INTO components_master (article, name, tv_model_id) VALUES (?, ?, ?)', [article, name, tv_model_id], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
...
app.post('/api/components-master/bulk', authenticateToken, (req, res) => {
    ...
    db_1.default.serialize(() => {
        ...
        const stmt = db_1.default.prepare('INSERT INTO components_master (article, name, tv_model_id) VALUES (?, ?, ?)');
        components.forEach((c) => {
            stmt.run(c.article, c.name, tv_model_id);
        });
        stmt.finalize();
        db_1.default.run('COMMIT', (err) => {
            if (err) {
...
app.delete('/api/components-master/:id', authenticateToken, (req, res) => {
    db_1.default.run('DELETE FROM components_master WHERE id = ?', [req.params.id], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
        res.json({ success: true });
    });
...
app.get('/api/tv/models', authenticateToken, (req, res) => {
    db_1.default.all('SELECT * FROM tv_models ORDER BY name ASC', [], (err, rows) => {
        if (err)
            return res.status(500).json({ error: err.message });
        res.json(rows);
    });
...
app.post('/api/tv/models', authenticateToken, (req, res) => {
    const { name, mn_keyword, label_sn_len, label_mn_len, label_ean_len, label_sn_fix, label_mn_fix, label_ean_fix, label_parsing_config, pallet_barcode_len, pallet_barcode_fix, pallet_parsing_config, pallet_keyword } = req.body;
    db_1.default.run('INSERT INTO tv_models (name, mn_keyword, label_sn_len, label_mn_len, label_ean_len, label_sn_fix, label_mn_fix, label_ean_fix, label_parsing_config, pallet_barcode_len, pallet_barcode_fix, pallet_parsing_config, pallet_keyword) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [name, mn_keyword, label_sn_len, label_mn_len, label_ean_len, label_sn_fix, label_mn_fix, label_ean_fix, label_parsing_config, pallet_barcode_len, pallet_barcode_fix, pallet_parsing_config, pallet_keyword], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
        res.json(Object.assign({ id: this.lastID }, req.body));
    });
});
app.put('/api/tv/models/:id', authenticateToken, (req, res) => {
    ...
    if (mn_keyword !== undefined) {
        updates.push('mn_keyword = ?');
        params.push(mn_keyword);
    }
    if (label_sn_len !== undefined) {
        updates.push('label_sn_len = ?');
        params.push(label_sn_len);
    }
    if (label_mn_len !== undefined) {
        updates.push('label_mn_len = ?');
        params.push(label_mn_len);
    }
    if (label_ean_len !== undefined) {
        updates.push('label_ean_len = ?');
        params.push(label_ean_len);
    }
    if (label_sn_fix !== undefined) {
        updates.push('label_sn_fix = ?');
        params.push(label_sn_fix);
    }
    if (label_mn_fix !== undefined) {
        updates.push('label_mn_fix = ?');
        params.push(label_mn_fix);
    }
    if (label_ean_fix !== undefined) {
        updates.push('label_ean_fix = ?');
        params.push(label_ean_fix);
    }
    if (label_parsing_config !== undefined) {
        updates.push('label_parsing_config = ?');
        params.push(label_parsing_config);
    }
    if (pallet_barcode_len !== undefined) {
        updates.push('pallet_barcode_len = ?');
        params.push(pallet_barcode_len);
    }
    if (pallet_barcode_fix !== undefined) {
        updates.push('pallet_barcode_fix = ?');
        params.push(pallet_barcode_fix);
    }
    if (pallet_parsing_config !== undefined) {
        updates.push('pallet_parsing_config = ?');
        params.push(pallet_parsing_config);
    }
    if (pallet_keyword !== undefined) {
        updates.push('pallet_keyword = ?');
        params.push(pallet_keyword);
    }
    if (updates.length === 0)
        return res.json({ success: true });
    params.push(req.params.id);
    db_1.default.run(`UPDATE tv_models SET ${updates.join(', ')} WHERE id = ?`, params, function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
        res.json({ success: true });
    });
...
app.delete('/api/tv/models/:id', authenticateToken, (req, res) => {
    ...
    db_1.default.serialize(() => {
        db_1.default.run('UPDATE lots SET tv_model_id = NULL WHERE tv_model_id = ?', [modelId]);
        db_1.default.run('DELETE FROM tv_models WHERE id = ?', [modelId], function (err) {
            if (err)
                return res.status(500).json({ error: err.message });
            res.json({ success: true });
        });
...
app.get('/api/tv/tests', authenticateToken, (req, res) => {
    db_1.default.all('SELECT * FROM tv_tests ORDER BY name ASC', [], (err, rows) => {
        if (err)
            return res.status(500).json({ error: err.message });
        res.json(rows);
    });
...
app.post('/api/tv/tests', authenticateToken, (req, res) => {
    ...
    db_1.default.run('INSERT INTO tv_tests (name, description) VALUES (?, ?)', [name, description || ''], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
...
app.put('/api/tv/tests/:id', authenticateToken, (req, res) => {
    const { name, description } = req.body;
    db_1.default.run('UPDATE tv_tests SET name = ?, description = ? WHERE id = ?', [name, description, req.params.id], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
        res.json({ success: true });
    });
...
app.delete('/api/tv/tests/:id', authenticateToken, (req, res) => {
    db_1.default.run('DELETE FROM tv_tests WHERE id = ?', [req.params.id], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
        res.json({ success: true });
    });
...
app.get('/api/breaks', authenticateToken, (req, res) => {
    db_1.default.all('SELECT * FROM breaks ORDER BY start_time ASC', [], (err, rows) => {
        if (err)
            return res.status(500).json({ error: err.message });
        res.json(rows);
    });
...
app.post('/api/breaks', authenticateToken, (req, res) => {
    ...
    db_1.default.run('INSERT INTO breaks (name, start_time, end_time) VALUES (?, ?, ?)', [name, start_time, end_time], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
        logAudit(req.user.id, 'CREATE_BREAK', { name, start_time, end_time });
        res.json({ id: this.lastID, name, start_time, end_time });
    });
});
app.put('/api/breaks/:id', authenticateToken, (req, res) => {
    const { name, start_time, end_time } = req.body;
    db_1.default.run('UPDATE breaks SET name = ?, start_time = ?, end_time = ? WHERE id = ?', [name, start_time, end_time, req.params.id], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
        logAudit(req.user.id, 'UPDATE_BREAK', { id: req.params.id, name, start_time, end_time });
        res.json({ success: true });
    });
});
app.delete('/api/breaks/:id', authenticateToken, (req, res) => {
    db_1.default.run('DELETE FROM breaks WHERE id = ?', [req.params.id], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
        logAudit(req.user.id, 'DELETE_BREAK', { id: req.params.id });
        res.json({ success: true });
    });
});
// --- SETTINGS & AUDIT ---
...
app.get('/api/kpi/facts', authenticateToken, (req, res) => {
    ...
    db_1.default.get('SELECT * FROM daily_kpi_facts WHERE date = ?', [date], (err, row) => {
        if (err)
            return res.status(500).json({ error: err.message });
        if (!row) {
...
app.get('/api/kpi/last-closed', authenticateToken, (req, res) => {
    db_1.default.get('SELECT * FROM daily_kpi_facts WHERE closed_at IS NOT NULL ORDER BY date DESC LIMIT 1', [], (err, row) => {
        if (err)
            return res.status(500).json({ error: err.message });
        if (!row) {
...
app.post('/api/kpi/facts', authenticateToken, (req, res) => {
    ...
    db_1.default.run('INSERT OR REPLACE INTO daily_kpi_facts (date, mes_fact, aql_plan, closed_at) VALUES (?, ?, ?, ?)', [date, mes_fact !== null && mes_fact !== void 0 ? mes_fact : 0, aql_plan !== null && aql_plan !== void 0 ? aql_plan : 0, closedAtStr], (err) => {
        if (err)
            return res.status(500).json({ error: err.message });
...
app.get('/api/settings', authenticateToken, (req, res) => {
    db_1.default.all('SELECT * FROM global_settings', [], (err, rows) => {
        if (err)
            return res.status(500).json({ error: err.message });
        const settings = {};
...
app.post('/api/settings/api-key/generate', authenticateToken, (req, res) => {
    ...
    db_1.default.run(`INSERT OR REPLACE INTO global_settings (key, value) VALUES ('api_key', ?)`, [newKey], (err) => {
        if (err)
            return res.status(500).json({ error: err.message });
...
app.put('/api/settings', authenticateToken, (req, res) => {
    ...
    db_1.default.serialize(() => {
        Object.keys(settings).forEach(key => {
            db_1.default.run('INSERT OR REPLACE INTO global_settings (key, value) VALUES (?, ?)', [key, settings[key]]);
        });
...
app.get('/api/admin/backup-zip', authenticateToken, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        // Fetch all lots
        const lots = yield new Promise((resolve, reject) => {
            db_1.default.all('SELECT * FROM lots', [], (err, rows) => {
                if (err)
                    reject(err);
                else
                    resolve(rows || []);
        ...
        for (const mod of modules) {
            const records = yield new Promise((resolve, reject) => {
                db_1.default.all(`SELECT * FROM ${mod}_logs`, [], (err, rows) => {
                    if (err)
                        reject(err);
                    else
                        resolve(rows || []);
            ...
            const parsedRecords = records.map(r => {
                ...
                return Object.assign({ id: r.id, lot_id: r.lot_id, user_id: r.user_id, date: r.date, status: r.status, timestamp: r.timestamp }, pData);
            });
...
const mesCache = new Map();
// Proxy for MES Dashboard to avoid CORS
app.post('/api/mes/proxy', authenticateToken, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    let { url } = req.body;
    if (!url)
    ...
    try {
        ...
        const response = yield (0, httpClient_1.requestWithRetry)(url, { timeout: 5000, retries: 2 });
        ...
        if (e instanceof httpClient_1.TimeoutError) {
...
app.post('/api/reports/panels-excel', authenticateToken, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const drawCell = (cellRef, text, options = {}) => {
...
const runDatabaseBackup = () => {
    const backupsDir = path_1.default.resolve(__dirname, '../backups');
    try {
        ...
        const tempBackupPath = path_1.default.join(backupsDir, `backup_temp_${Date.now()}.sqlite`);
        const finalBackupPath = path_1.default.join(backupsDir, backupFileName);
        const latestBackupPath = path_1.default.join(backupsDir, 'backup_latest.sqlite');
        logger.info('Запуск фонового бэкапа базы данных (SQLite VACUUM INTO)...');
        db_1.default.run(`VACUUM INTO ?`, [tempBackupPath], (err) => {
            if (err) {
                logger.error(`Сбой автоматического бэкапа (VACUUM INTO): ${err.message}`);
                if (fs_1.default.existsSync(tempBackupPath)) {
                    try {
            ...
            try {
                ...
                if (fs_1.default.existsSync(finalBackupPath)) {
                    fs_1.default.unlinkSync(finalBackupPath);
                }
                fs_1.default.renameSync(tempBackupPath, finalBackupPath);
                // Also copy/link to backup_latest.sqlite for backward compatibility
                if (fs_1.default.existsSync(latestBackupPath)) {
                    fs_1.default.unlinkSync(latestBackupPath);
                }
                fs_1.default.copyFileSync(finalBackupPath, latestBackupPath);
                ...
                const backupFiles = files
                    ...
                    .map(f => ({ name: f, path: path_1.default.join(backupsDir, f), mtime: fs_1.default.statSync(path_1.default.join(backupsDir, f)).mtime.getTime() }))
...
const autoCloseShift = () => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        const today = new Date().toISOString().split('T')[0];
        // 1. Fetch settings
        let mesUrl = (yield getSetting('mes_dashboard_url')) || 'http://192.168.210.210:8000/tablo/lines/1/dashboard/';
        if (mesUrl && !mesUrl.startsWith('http'))
            mesUrl = 'http://' + mesUrl;
        const shiftConfigStr = yield getSetting('oqa_shift_config');
        let shiftConfig = {};
        try {
            if (shiftConfigStr)
                shiftConfig = JSON.parse(shiftConfigStr);
        }
        catch (e) { }
        ...
        try {
            process.env.NODE_TLS_REJECT_UNAUTHORIZED = '0';
            const response = yield (0, httpClient_1.requestWithRetry)(mesUrl, { timeout: 5000, retries: 2 });
            const html = yield response.text();
            const jsonMatch = html.match(/initialDashboardData\s*=\s*JSON\.parse\(\s*'([\s\S]*?)'\s*\)/);
            if (jsonMatch && jsonMatch[1]) {
                try {
                    const unescaped = jsonMatch[1].replace(/\\u([0-9a-fA-F]{4})/g, (match, grp) => String.fromCharCode(parseInt(grp, 16)));
                    const data = JSON.parse(unescaped);
                    const fact = (_a = data.metrics) === null || _a === void 0 ? void 0 : _a.curr_device_count;
        ...
        if (factVal === 0) {
            const dbFact = yield new Promise((resolve) => {
                db_1.default.get('SELECT mes_fact FROM daily_kpi_facts WHERE date = ?', [today], (err, row) => resolve(row));
            });
        ...
        const closedAtStr = new Date().toLocaleString('ru-RU');
        yield new Promise((resolve, reject) => {
            db_1.default.run('INSERT OR REPLACE INTO daily_kpi_facts (date, mes_fact, aql_plan, closed_at) VALUES (?, ?, ?, ?)', [today, factVal, planVal, closedAtStr], (err) => {
                if (err)
                    reject(err);
                else
                    resolve();
